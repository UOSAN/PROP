...
%[typ,num,pre,maxTime,totTime,rep,stpEvt,bg,st,bgFile,stFile,hshift,vshift,tag]= textread( tab_delimited_text_file, '%c %d %f %f %f %d %s %c %c %s %s %d %d %s','delimiter', '\t', 'whitespace', '', 'commentstyle', 'matlab' );
%
...
experiment_notes='This is the script for the PRP task, updated 7-19-18.';
...
% if (resource_path(1) ~= '/') || strcmp(resource_path(1:2),'~/') %if resource_path does not start with a slash we assume that the resource dir is relative to the current dir
%     tdfile_path
%     resource_path
...
if length(typ) ~= length(stFile)
    fprintf('Warning: Your typ vector and your stFile vector do not match in length\n');
end;
...
for c = 1:length(stFile)
    ...
    if isempty(typ(c))
        fprintf('In the future, you should specify a typ for trial %d.  Reverting to default (s)\n',c);
        typ(c) = 's';
    end;
...
for i=1:length(stFile)
    ...
    elseif ~isempty(stFile{i})
        if stFile{i}(1)=='"' && stFile{i}(end)=='"'    
            stFile{i} = stFile{i}(2:end-1);
        end
    end;
...
for j = 1:number_of_trials,
    ...
    if st(i) == 'm' %primary stimulus is a movie        
        ...
        %p = pause trial
        while (typ(i) == 'p' && no_response_yet == 1) || (typ(i) ~= 'p' && no_response_yet == 1 && GetSecs < start_time + stimulus_duration(i)),
            [keyIsDown,secs,keyCode]=KbCheck(inputDevice);
            if keyIsDown, %key is pressed
                key_presses.key{length(key_presses.key)+1} = KbName(keyCode);
                key_presses.time(length(key_presses.time)+1) = GetSecs - experiment_start_time;
    ...
    if GetSecs < pre_start_time+totTime(i) && typ(i) ~= 'p'
        eval(bg_display);
        Screen('Flip',w);
        while GetSecs < pre_start_time+totTime(i) && typ(i) ~= 'p'
